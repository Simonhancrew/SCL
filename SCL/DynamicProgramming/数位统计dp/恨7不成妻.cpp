...
/*
    ...
    2. 各位数字之和不是7的倍数
    3. 整个数不是7的倍数
    不难想到一个状态定义: f[i][j][a][b],当前一共i位，最高位是j，当前这个数mod 7 本身是a
    这个数的各位之和mod 7的结果是b，当前情况下的平方和
            ...
            for(int a = 0;a < 7;a++){
                for(int b = 0;b < 7;b++){
                    for(int k = 0;k <= 9;k++){
                        ...
                        v1.s1 = mod(v1.s1 + v2.s1 + j * (power % P) % P * v2.s0,P); // 放置溢出，power已经大了，要先取模
...
int dp(LL n){
    ...
    for(int i = nums.size() - 1;i >= 0;i--){
        ...
        for(int j = 0;j < x;j++){
            ...
            // 做一次完全平方展开
            res = mod(
                res + (last_a % P) * (last_a % P) % P * power9[i + 1] % P * power9[i + 1] % P * v.s0 % P + 
                v.s2 + 
