...
/*
    ...
    重新理解seg
   tree，modify的时候入参永远是常量,修改的区间是[L,R]，往下走去查询线段树的
...
class CountIntervals {
 ...
 public:
  ...
  void add(int L, int R) {
    if (sum == r - l + 1) return;  // 当前区间完全覆盖，没必要往下了
    if (L <= l && r <= R) {
      fill();
      return;
    }
    int mid = (l + r) >> 1;
    if (lhs == nullptr) lhs = new CountIntervals(l, mid);
    if (rhs == nullptr) rhs = new CountIntervals(mid + 1, r);
