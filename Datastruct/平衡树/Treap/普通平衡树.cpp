...
void right_rotate(int &p) { // 画图就很清晰，注意pushup先更新底下的再更新上面的
    int q = tr[p].l;
...
void insert(int &p,int key) { // 递归的插入，注意判断val的值之后旋转
    if(!p) p = new_node(key);
...
void del(int& p,int key) { // 子节点就直接删除，不是子节点就把他旋转到子节点之后删除
    if(!p) return;
    if(tr[p].key == key) {
        if(tr[p].cnt > 1) tr[p].cnt--;
        else if(tr[p].l || tr[p].r) // 不是叶子节点
        {
            if(!tr[p].r || tr[tr[p].l].val > tr[tr[p].r].val) { // 可以右旋的情况，没有右儿子或者左儿子的val更大，可以做爹
                right_rotate(p);
                del(tr[p].r,key);
            }
            else{
